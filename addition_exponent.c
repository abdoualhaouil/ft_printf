...
/*   Updated: 2019/11/18 18:24:58 by aalhaoui         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
char	*add_zero_exponent(char *n1, char *n2)
{
	...
	str = zero(r);
	if (i < j)
		str = ft_strjoin(str, n1);
	else
		str = ft_strjoin(str, n2);
	return (str);
}
...
char	*addition_exponent(char *n1, char *n2)
{
	...
	i = ft_strlen(n1);
	j = ft_strlen(n2);
	(i != j) && (i > j) && (n2 = add_zero_exponent(n1, n2));
	(i != j) && (i < j) && (n1 = add_zero_exponent(n1, n2));
	i = (i > j) ? i + 1 : j + 1;
	res = ft_strnew(i);
	remainder = 0;
	while (--i > 0)
